...
func remove_rider() -> Node2D:
	for y in range(BOARD_SIZE):
		for x in range(BOARD_SIZE):
			var piece = board[y][x]
			if piece != null and ("is_white" in piece):
				board[y][x] = null
				remove_child(piece)
				return piece
	return null
...
func receive_rider(rider_node: Node2D):
	...
	var target_pixel_pos = Vector2((target_x * TILE_SIZE) + (TILE_SIZE / 2), (target_y * TILE_SIZE) + (TILE_SIZE / 2))
	
	add_child(rider_node)
	rider_node.position = target_pixel_pos
